...
export const formatThaiDate = (date: Date | string | number): string => {
  if (!date) return "N/A"; // ตรวจสอบว่ามีวันที่หรือไม่
  if (typeof date !== "string" && typeof date !== "number") {
    return "N/A"; // ถ้าไม่ใช่ string หรือ number ให้คืนค่า N/A
  }
  const d = new Date(date); // แปลงเป็น ISO เพื่อความแม่นยำในการจัดการเวลา;
  return new Intl.DateTimeFormat("th-TH", {
    ...
    second: "2-digit",
    hour12: false,
    timeZone: "Asia/Bangkok", // 🚩 ระบุให้ใช้เวลาประเทศไทยเสมอ
  }).format(d);
};
